...
p1 <- ggplot(data[1:5,], aes(x = Methods, y = HR, colour = type), size = 5) + 
  ...
  annotate(geom = "text", x = c(c(1.5 + 5 * (0), 4 + 5 * (0))), y = 1 - 1.75 / 100, 
           label = c(rep(c("Regression"), 1), rep(c("Causal"), 1)), size = 8) +
...
p2 <- ggplot(data[6:10, ], aes(x = Methods, y = HR), size = 5) + 
  ...
  geom_errorbar(aes(ymin = lower_CI, ymax = upper_CI), width = 0.2, size = 1, colour = "darkblue") +
  geom_vline(xintercept = 2.5, linetype = "dashed", size = 0.8) +
  annotate(geom = "text", x = seq_len(nrow(data)), y = 1 - 1.875 / 100, label = data$method, size = 8) +
  annotate(geom = "text", x = c(3), y = 1 + 52.5 / 100,
           label = c(expression(paste("Medicare Enrollees Exposed to PM"[2.5] <= 12 , mu, g/m^3, "(2000-2016)"))), size = 8) +
  annotate(geom = "text", x = c(c(1.5 + 5 * (0), 4 + 5 * (0))), y = 1 - 4.375 / 100,
           label = c(rep(c("Regression"), 1), rep(c("Causal"), 1)), size = 8) +
...
p1 <- ggplot(balance, aes(x=Correlation, y=Covariates, colour=Implementations)) + 
  scale_y_discrete(limit = 1:16,
                   labels = c("Calender Year", "Census Region", "Mean BMI", "% Ever Smoked", 
                              "% Hispanic", "% Black", "Median Household Income", "Median Home Value",
