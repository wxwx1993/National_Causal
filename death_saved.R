...
dead_personyear <- aggregate(cbind(aggregate_data$dead,
                                   aggregate_data$time_count), 
                             by = list(aggregate_data$zip, aggregate_data$year),
                             FUN = sum)
pm <- aggregate(aggregate_data$pm25_ensemble,
                by = list(aggregate_data$zip, aggregate_data$year),
                FUN = min)
