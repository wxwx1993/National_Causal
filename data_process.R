...
# Main analysis
covariates <- aggregate(national_merged2016[, c(12:27)], 
                        by = list(national_merged2016$zip, national_merged2016$year), 
                        FUN = min)
...
dead_personyear <- aggregate(cbind(national_merged2016$dead,
                                   national_merged2016$time_count), 
                             ...
                             FUN = sum)
confounders <- aggregate(national_merged2016[,c(12:27)],
                         by=list(national_merged2016$zip,
                                 national_merged2016$year,
                                 national_merged2016$sex,
                                 national_merged2016$race,
...
aggregate_data.list <- split(aggregate_data,
                             list(aggregate_data$sex,
                                  aggregate_data$race,
                                  aggregate_data$dual,
                                  aggregate_data$entry_age_break,
                                  aggregate_data$followup_year))
...
GPS_mod <- xgboost(data = data.matrix(covariates[, c(4:18)]),
                   label = covariates$pm25_ensemble,
                   nrounds = 50)
mod_sd <- sd(covariates$pm25_ensemble - predict(GPS_mod, data.matrix(covariates[, c(4:18)])))
feature_names <- GPS_mod$feature_names
covariates$GPS <- dnorm(covariates$pm25_ensemble,
                        mean = predict(GPS_mod, data.matrix(covariates[, c(4:18)])),
...
GPS_mod <- xgboost(data = data.matrix(covariates[, c(4:13,18:19)]),
                   label = covariates$pm25_ensemble,
                   nrounds = 50)
mod_sd <- sd(covariates$pm25_ensemble - predict(GPS_mod, data.matrix(covariates[, c(4:13,18:19)])))
feature_names <- GPS_mod$feature_names
covariates$GPS <- dnorm(covariates$pm25_ensemble,
                        mean = predict(GPS_mod, data.matrix(covariates[, c(4:13,18:19)])),
...
write.foreign(national_merged2016, 
              datafile = "./national_merged2016.csv", 
