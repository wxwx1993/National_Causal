...
IPTW_gnm_raw <- gnm(dead ~ pm25_ensemble + offset(log(time_count)),
                    eliminate = (as.factor(sex):as.factor(race):as.factor(dual):as.factor(entry_age_break):as.factor(followup_year)),
                    data = aggregate_data,
                    family = poisson(link = "log"),
                    weights = IPW)
...
GPScova.fun.dose <- function(simulated.data, GPS_mod, a,model1 = flexible_model1) {
  ...
  data.a <- data.frame(cbind(cbind(pm25_ensemble = a,
                                   GPS = p.a), 
                             simulated.data[, c(1:7, 9)]))
  data.a$dead <- predict(model1, data.a, type="response")

  data.a <- aggregate(cbind(data.a$dead,
                            data.a$time_count,
                            data.a$pm25_ensemble), 
                      by = list(data.a$sex,
                                data.a$race,
                                data.a$dual,
                                data.a$entry_age_break,
                                data.a$followup_year), 
                      FUN = mean)
...
flexible_model1 <- bam(dead ~ pm25_ensemble + GPS + pm25_ensemble*GPS + I(GPS^2) +
                         (as.factor(sex) + as.factor(race) + as.factor(dual) + as.factor(entry_age_break) + as.factor(followup_year))^5 +
                         offset(log(time_count)),
                       data = aggregate_data,
                       family = poisson(link = "log"),
                       chunk.size = 5000,
...
GPScova <- data.table(Reduce(rbind, 
                             mclapply(a.vals + delta_n / 2, GPScova.fun.dose,
                                      simulated.data = aggregate_data,
                                      GPS_mod = GPS_mod,
                                      model1 = flexible_model1,
                                      mc.cores = 1)))
...
GPScova_gam <- gam(log(dead) ~ s(pm25_ensemble) + 
